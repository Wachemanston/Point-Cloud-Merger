...
int main(int, char* argv[])
{
    ...
    std::string mkdirCommand = std::string("mkdir ") + outputPath + basename;
    ...
    if (pathsAugmentData.size() < offset)
    {
        std::cerr << "No enough augment data(at least: " << offset << ")\n";
        return 1;
    }
    ...
    for (size_t i = 0; i < pathsRealData.size(); ++i)
    {
        ...
        pcdWriter.writeASCII(outputPath + basename + std::string("/") + file_name, combinedPointCloud);
    }
